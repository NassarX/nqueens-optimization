...
from app.charles import sim_annealing
...
class NQueensSimulatedAnnealing:
    """A class to represent the N-Queens Simulated Annealing Algorithm.
...
def main():
    parser = argparse.ArgumentParser(description='N-Queens Simulated Annealing Algorithm')
    parser.add_argument('-n', '--n-queen', type=int, default=N_QUEEN_CONST,
                        help='Number of queens')
    args = parser.parse_args()

    nQueensHC = NQueensSimulatedAnnealing(dimension=args.n_queen)
    start_time = datetime.now()
    try:
        start_time = datetime.now()
        nQueensHC.search()
        ...
        end_time = datetime.now()
        representation = ""
        representation += "N-Queens Simulated Annealing Algorithm\n"
        representation += "==========================\n"
